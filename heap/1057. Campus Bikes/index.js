...
const assignBikes = function(workers, bikes) {
  ...
  for (let i = 0; i < n; i++) {
    ...
    for (let j = 0; j < m; j++) {
      ...
      let dist = Math.abs(bike[0] - worker[0]) + Math.abs(bike[1] - worker[1]);
      pq.offer([dist, i, j]);
    }
  }

  let visitedBikes = Array(n).fill(false);
  let ans = Array(m).fill(-1);
  let cnt = 0;

  while (cnt < m) {
    let [_, bikeIndex, workerIndex] = pq.poll();

    if (ans[workerIndex] == -1 && !visitedBikes[bikeIndex]) {
