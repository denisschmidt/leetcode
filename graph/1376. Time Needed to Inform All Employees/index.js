...
const numOfMinutes = (n, headID, manager, informTime) => {
  ...
  while (queue.length) {
    ...
    for (let k = 0; k < size; k++) {
      let [u, time] = queue.shift();

      for (let v of parent.get(u)) {
        queue.push([v, time + informTime[u]]);
      }

      // Если дошли до листа
      // Ответ - это максимальное время, когда листовой узел должен быть проинформирован
      if (parent.get(u).length == 0) {
