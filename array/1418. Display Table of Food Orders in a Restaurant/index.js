...
const displayTable = orders => {
  ...
  for (let [name, table, dishes] of orders) {
    if (!map.has(table)) map.set(table, new Map());

    set.add(dishes);
    map.get(table).set(dishes, map.get(table).has(dishes) ? map.get(table).get(dishes) + 1 : 1);

    t.add(table);
  }

  let dishesPosition = new Map();
  let dishes = Array.from(set).sort();

  dishes.forEach((x, index) => dishesPosition.set(x, index));

  let tableList = Array.from(t).sort((a, b) => a - b);

  let res = [];

  res.push(['Table', ...dishes]);

  for (let id of tableList) {
    let item = Array(dishes.length + 1).fill('0');
    item[0] = id;

    for (let [dish, cnt] of map.get(id)) {
      let pos = dishesPosition.get(dish);
      item[pos + 1] = cnt.toString();
    }
