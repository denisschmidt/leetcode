...
2. getTweetCountsPerFrequency(string freq, string tweetName, int startTime, int endTime)
  ...
  [startTime, startTime + delta*1>,  [startTime + delta*1, startTime + delta*2>, [startTime + delta*2, startTime + delta*3>, ... , [startTime + delta*i, min(startTime + delta*(i+1), endTime + 1)>  for some non-negative number i and delta (which depends on freq).  
...
class TweetCounts {
  ...
  getTweetCountsPerFrequency(freq, tweetName, startTime, endTime) {
    let delta = this.getFreqSeconds(freq);
    ...
    let cnt = 0;
    let res = [];
    let list = this.map.get(tweetName);

    while (end <= endTime) {
      cnt = 0;
      start = startTime + delta * i;
      end = Math.min(startTime + delta * (i + 1), endTime + 1);
