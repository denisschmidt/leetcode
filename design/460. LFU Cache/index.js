/*
Design and implement a data structure for Least Frequently Used (LFU) cache.(Наименее часто используемые)
...
class FrequentList extends DoublyLinkedList {
  ...
  promote(dataNode) {
    let frequentNode = dataNode.frequentNode;

    frequentNode.dataList.remove(dataNode);

    if (frequentNode.prev.frequent !== frequentNode.frequent + 1) {
      let newFrequentNode = new FrequentNode(frequentNode.frequent + 1);
