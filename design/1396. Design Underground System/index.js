...
class UndergroundSystem {
  ...
  checkIn(id, stationName, t) {
    ...
    this.user.get(id).push({ start: t, end: null, from: stationName, to: null });
  }

  checkOut(id, stationName, t) {
    for (let x of this.user.get(id)) {
      if (x.to == null && x.start < t) {
        x.to = stationName;
        x.end = t;

        if (!this.station.has(x.from + x.to)) this.station.set(x.from + x.to, []);
        this.station.get(x.from + x.to).push(x.end - x.start);
  ...
  getAverageTime(startStation, endStation) {
    let cnt = 0;
    let sum = 0;
    for (let time of this.station.get(startStation + endStation)) {
      cnt++;
      sum += time;
    }
