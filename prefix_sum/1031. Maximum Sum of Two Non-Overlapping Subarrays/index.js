...
const maxSumTwoNoOverlap = (A, L, M) => {
  return Math.max(calcSum(A, L, M), calcSum(A, M, L));

  function calcSum(nums, len1, len2) {
    let n = nums.length;
    let max = 0;

    for (let i = 0; i < n; i++) {
      let sum1 = nums.slice(i, i + len1).reduce((acc, v) => acc + v, 0);

      for (let j = i + len1; j + len2 - 1 < n; j++) {
        let sum2 = nums.slice(j, j + len2).reduce((acc, v) => acc + v, 0);
