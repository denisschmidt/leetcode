...
class LogSystem {
  ...
  put(id, timestamp) {
    let st = timestamp.split(':');

    // минус 1 нужен таккак в js месяц начинается с 0
    st[1] = parseInt(st[1] - 1);
    let date = new Date(...st).getTime();
    this.map.set(date, id);
  }

  granularity(s, gra, end) {
    let tmp = ['Year', 'Month', 'Day', 'Hour', 'Minute', 'Second'];
    let id = tmp.indexOf(gra);
    let res = s.split(':').slice(0, id + 1);
  ...
  retrieve(s, e, gra) {
    let ans = [];
    let start = this.granularity(s, gra, false);
    let end = this.granularity(e, gra, true);
