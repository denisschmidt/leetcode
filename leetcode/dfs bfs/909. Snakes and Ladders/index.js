...
const snakesAndLadders = board => {
  ...
  let target = n * n;
  let distanse = Array(target + 1).fill(Number.MAX_VALUE);
  distanse[1] = 0;

  dfs(1);

  return distanse[target] == Number.MAX_VALUE ? -1 : distanse[target];
  ...
  function dfs(num) {
    ...
    for (let k = 1; k <= 6; k++) {
      ...
      if (newNum <= target) {
        let value = getBoardValue(newNum);
        let d = distanse[num] + 1;

        if (value == -1) {
          if (distanse[newNum] > d) {
            distanse[newNum] = d;
            dfs(newNum);
          }
        } else {
