...
You just need to ensure that an N-ary tree can be serialized to a string and this string
...
class Codec {
  ...
  deserialize(root) {
    ...
    if (root.children.length > 0) {
      result.left = this.deserialize(root.children[0]);
    }

    let curr = result.left;

    for (let i = 1; i < root.children.length; i++) {
      curr.right = this.deserialize(root.children[i]);
      curr = curr.right;
    }
  ...
  serialize(root) {
    ...
    while (curr) {
      result.children.push(this.serialize(curr));
      curr = curr.right;
    }
