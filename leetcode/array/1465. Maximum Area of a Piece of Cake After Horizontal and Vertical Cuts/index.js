...
const maxArea = (h, w, horizontalCuts, verticalCuts) => {
  ...
  let width = Math.max(verticalCuts[0], w - verticalCuts[verticalCuts.length - 1]);
  let heigth = Math.max(horizontalCuts[0], h - horizontalCuts[horizontalCuts.length - 1]);
  ...
  for (let i = 1; i < horizontalCuts.length; i++) {
    heigth = Math.max(heigth, horizontalCuts[i] - horizontalCuts[i - 1]);
  }

  return (heigth * width) % (1e9 + 7);
};
