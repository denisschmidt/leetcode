...
as another replacement operation i = 2, x = "ec", y = "ffff", this second operation does nothing because in the original string S[2] = 'c', 
...
// Time O(NLogN + N * K)
// Space O(1)
const findReplaceString = (str, indexes, sources, targets) => {
  // при сортировке по убыванию дельта разницы нам больше не нужна !
  indexes = indexes.map((start, index) => ({ start, index })).sort((a, b) => b.start - a.start);

  for (let i = 0; i < indexes.length; i++) {
    let { start, index } = indexes[i];
    const source = sources[index];
    const target = targets[index];

    if (str.substring(start, start + source.length) === source) {
      str = str.substring(0, start) + target + str.substring(start + source.length);
    }
  }
...
// Time O(NLogN + N * K)
// Space O(1)
const findReplaceString2 = (str, indexes, sources, targets) => {
  let delta = 0;
  indexes = indexes.map((start, index) => ({ start, index })).sort((a, b) => a.start - b.start);
  ...
  for (let i = 0; i < indexes.length; i++) {
    ...
    const prefix = str.substring(start);

    let prev = '';
    if (start !== 0) {
      prev = str.substring(0, start);
    }

    if (prefix.startsWith(source)) {
      str = prev + target + prefix.substring(source.length);
      // сдвиг массива после замены
