...
const countPairs = (root, distance) => {
  ...
  function dfs(node) {
    ...
    // store distances of left leaf nodes
    let left = dfs(node.left);

    // store distances of right leaf nodes
    let right = dfs(node.right);

    // update distances
    left = left.map(x => x + 1).filter(x => x <= distance);
    right = right.map(x => x + 1).filter(x => x <= distance);

    // on node x check leaf nodes which satisfying the condition
    // update global ans
    for (let d1 of left) {
      for (let d2 of right) {
        if (d1 + d2 <= distance) ans++;
      }
    }
