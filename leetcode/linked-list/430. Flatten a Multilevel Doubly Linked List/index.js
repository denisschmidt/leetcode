...
const flatten = head => {
  ...
  function helper(cur, parent) {
    if (cur == null) return cur;

    if (cur.child) {
      helper(cur.child, cur);
    }

    helper(cur.next, parent);

    // в данном кейсе у нас есть родитель
    // cur содержит ноду которая является последней в списке parent.child
    if (parent && parent.child) {
      if (parent.next) {
        parent.next.prev = cur;
      }
