/*
  ...
  Зачем нужна таблица префиксов ? 

  Простой пример демонстрирующий, как алгоритм KMP использует таблицу префиксов для пропуска ранее согласованных префиксов.
   
  A: aabaabaac
  B: aabaac 
  
  Начинаем сопоставление с A [i = 0] и B [j = 0] сопоставим «aabaa» и остановимся на B [j = 5] ('c' != 'b'). 
  Без таблицы префиксов нам придется снова сопоставлять буквы, начиная с A [i = 1] и B [j = 0]. 
  
  prefix = suffix = 'aabaa' нет смысла рассчитывать вхождение 'aabaa' снова
  Поэтому мы берем точку j и получаем новый индекс точки j = 0 с которой и начинаем поиск.

  ////////////////////////////////////////////////////////////////////////////////////////////////////////////////

  Пример:

  Расчитаем таблицу префиков для паттерна ['a c a c a b a c a c a b a c a c a c']
